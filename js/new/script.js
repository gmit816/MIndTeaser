...
function initQuestions(options) {
	...
	if ($corrAm.length)
	{
		var _updateCorrect = function() {
			...
			ll.each(function(index) {
				var label = getLabel(index, ll.length);
				$(this).attr('placeholder', label);							// Update placeholder
				$("label[for=" + this.id + "]").text(label);				// Update Label
				$(this).closest('.fieldgroup').find('h2').text(label);		// Update Title
			});
		};
	...
	$('.createanswers [data-more]').hide();
	$(".createanswers").parent().on('click', "#moreAnswers", function (ev) {
		ev.preventDefault();
		$('.createanswers [data-more]:hidden:first').show();
		if (!$('.createanswers [data-more]:hidden').size()) {
			$(this).hide();
		}
...
function initAssessmentOptionsQuestion(options) {
	...
	var addCategoryOption = function(category) {
		var container = $(category).find('.categories > .items');
...
function initUserManagement()
{
	...
	$dialog.on('submit', 'form', function(e) {
		e.preventDefault();
		$.fn.yiiGridView.update('users-table', {
			type: 'POST',
			data: $(this).serialize()
		});
	});	
	...
	$('.add-users').click(function(e) {
		...
		if (!$dialog.data('loaded')) {
			$dialog.load($(this).attr('href')).data('loaded', true);
		}
		$dialog.dialog('open');
	});
}
