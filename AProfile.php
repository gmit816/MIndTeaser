...
<head>
    ...
    <meta name="title" content="My dashboard">
                                ...
                                <nav>
                                    <ul class="menu level1">
                                        <li class="first haschildren">
                                            <a href="ADashboard.php">Dashboard</a></li>
                                        <li class="haschildren">
                                            <a href="AQuiz.php">Quiz</a></li>
                                        <li class="haschildren">
                                            <a href="AFeedbackView.php">User Feedback</a></li>
                                        <li class="hier haschildren">
                                            <a href="AProfile.php">Profile</a></li>
                                    </ul>
                                </nav>
                            </div>
                        </div>
            ...
            <div class="wrapper clearfix">
                <div class="rowbox managecontainer">
                    <div class="row border_bottom manageheader">
                        <div class="col3">
                            <div class="contentbox">
                                <strong class="heading h3">Profile</strong>
                            </div>
                        </div>
                        <div class="col9">
                            <div class="row">
                                <div class="col6">
                                    <div class="contentbox noLeftPadding">
                                        <h1><?php echo $_SESSION["First_name"];?>'s Profile</h1>
                                    </div>
                                </div>
                                <div class="col3">
                            ...
                            <div class="navigation quiznav">
                                <nav>
                                    <ul class="menu level1">
                                        ...
                                        <li><a href="AQuiz.php">Quiz</a></li>
                                        <li><a href="AFeedbackView.php">User Feedback</a></li>
                                        <li class="hier open"><a href="AProfile.php">Profile</a></li>
                                    </ul>
                                </nav>
                            </div>
                        </div>
                        <div class="col9">
                            <div class="contentbox noLeftPadding">
                                <div class="pagecontent">
                                    <div class="intro">
                                        <p>Welcome to your personal profile. From this altitude you can checkyour created quiz and their attenpts and played quiz and score too.</p>
                                    </div>
                                </div>
                                <h3><?php echo $_SESSION["First_name"];?>'s Personal Details:</h3>
                                <?php
                                ...
                                $selectSQL = "SELECT * FROM `user_master` WHERE User_id='{$_SESSION["User_id"]}'";
                                ...
                                <table>
                                    ...
                                    if (mysql_num_rows($selectRes) == 0) {
                                        ...
                                        while ($row = mysql_fetch_assoc($selectRes)) {
                                            echo "<tr><th>Name:</th><td>{$row['First_name']}&nbsp;{$row['Last_name']}</td><td></td></tr>
                                            <tr><th>Email_Id:</th><td>{$row['Email_ID']}</td><td></td></tr>
                                            <tr><th>User_Id:</th><td>{$row['User_id']}</td><td></td></tr>";
                                        }
                                    }
                                    }
                                    ?>
                                ...
                                <h3><?php echo $_SESSION["First_name"];?>'s Created Quiz:</h3>
                                <?php
                                $con=mysql_connect("localhost","root","");
                                if(!$con)
                                {
                                ...
                                <table class="quizindex">
                                    ...
                                    <tr>
                                        ...
                                        <th class="alignright">No.of Attempts</th>
                                    ...
                                    if (mysql_num_rows($selectRes) == 0) {
                                        ...
                                        while ($row = mysql_fetch_assoc($selectRes)) {
                                            $mit="SELECT * FROM `quiz_played` WHERE Quiz_id='{$row["Quiz_id"]}'";
                                            $r=mysql_query($mit,$con);
                                            $m=mysql_num_rows($r);
                                            echo "<tr><td>{$row['Quiz_name']}</td><td class=\"alignright\">{$row['Quiz_id']}</td><td class=\"alignright\">$m</td></tr>\n";
                                        }
                                    }
                                    }
                                    ?>
                                ...
                                <br/>
                                <br/>
                                <h3><?php echo $_SESSION["First_name"];?>'s Played Quiz:</h3>
                                <?php
                                $con=mysql_connect("localhost","root","");
                                if(!$con)
                                {
                                ...
                                mysql_select_db("mindteaser",$con);
                                # Prepare the SELECT Query
                                $selectSQL = "SELECT * FROM `quiz_played` WHERE User_id='{$_SESSION["User_id"]}' ORDER BY Score DESC";
                                # Execute the SELECT Query
                                if( !( $selectRes = mysql_query( $selectSQL ) ) ){
                                    echo 'Retrieval of data from Database Failed - #'.mysql_errno().': '.mysql_error();
                                }else{
                                ...
                                <table class="quizindex">
                                    ...
                                    if (mysql_num_rows($selectRes) == 0) {
                                        ...
                                        while ($row = mysql_fetch_assoc($selectRes)) {
                                            $mit="SELECT * FROM `quiz_master` WHERE Quiz_id='{$row["Quiz_id"]}'";
                                            $r=mysql_query($mit,$con);
                                            if(mysql_num_rows($r)==1) {
                                                $row1 = mysql_fetch_array($r);
                                                echo "<tr><td>{$row1['Quiz_name']}</td><td class=\"alignright\">{$row['Quiz_id']}</td><td class=\"alignright\">{$row['Score']}</td></tr>\n";
                                            }
                                        }
                                    }
                                    }
